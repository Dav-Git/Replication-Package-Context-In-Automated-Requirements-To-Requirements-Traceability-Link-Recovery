 ...
 * <p>
 * All preprocessors have access to a shared {@link edu.kit.kastel.sdq.context.ContextStore} via the protected {@code contextStore} field,
...
public abstract class Preprocessor {
    ...
    public static Preprocessor createPreprocessor(ModuleConfiguration configuration, ContextStore contextStore) {
        return switch (configuration.name().split(Configuration.CONFIG_NAME_SEPARATOR)[0]) {
            case "sentence" -> new SentencePreprocessor(configuration, contextStore);
            case "code" ->
                throw new IllegalArgumentException("Not required for R2R TLR.");
            case "model" ->
                throw new IllegalArgumentException("Not required for R2R TLR.");
            ...
            case "paraphrase" -> new ParaphrasePreprocessor(configuration, contextStore);
