...
public final class Element extends Knowledge {
    ...
    /**
     * Retrieves the artifact element from a given element by traversing up the hierarchy
     * until it reaches the granularity level of artifact (0).
     *
     * @param element The element from which to retrieve the original artifact.
     * @return The artifact element.
     * @throws IllegalArgumentException if the element has a negative granularity.
     */
    public static Element getArtifactElement(Element element) {
        final int ARTIFACT_GRANULARITY = 0;
        final var original = element;
        if (element.getGranularity() < ARTIFACT_GRANULARITY) {
            throw new IllegalArgumentException("Unclear semantics of negative granularity in element: " + element);
        }
        while (element.getGranularity() > ARTIFACT_GRANULARITY) {
            element = element.getParent();
            if (element == null) {
                throw new IllegalStateException("No parent with granularity 0 found for " + original.getIdentifier());
            }
        }
        return element;
    }
