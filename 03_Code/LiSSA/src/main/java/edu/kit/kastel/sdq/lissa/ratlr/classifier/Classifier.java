...
import static edu.kit.kastel.sdq.lissa.ratlr.classifier.EmbeddingsAsContextClassifier.EMBEDDINGS_AS_CONTEXT_CLASSIFIER_NAME;
import static edu.kit.kastel.sdq.lissa.ratlr.classifier.ExternalTraceLinksAsContextClassifier.EXTERNAL_TRACE_LINKS_AS_CONTEXT_CLASSIFIER_NAME;
...
public abstract class Classifier {
    ...
    public static Classifier createClassifier(ModuleConfiguration configuration, ContextStore contextStore) {
        return switch (configuration.name().split(CONFIG_NAME_SEPARATOR)[0]) {
            ...
            case EMBEDDINGS_AS_CONTEXT_CLASSIFIER_NAME ->
                    new EmbeddingsAsContextClassifier(configuration, contextStore);
            case EXTERNAL_TRACE_LINKS_AS_CONTEXT_CLASSIFIER_NAME ->
                    new ExternalTraceLinksAsContextClassifier(configuration, contextStore);
