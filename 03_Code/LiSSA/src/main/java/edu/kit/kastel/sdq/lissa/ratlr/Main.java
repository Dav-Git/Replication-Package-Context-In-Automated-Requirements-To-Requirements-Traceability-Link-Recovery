...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        LissaConfiguration lissaConfiguration = new ObjectMapper().readValue(configFile, LissaConfiguration.class);
        CacheManager.setCacheDir(lissaConfiguration.cacheDir());

        ContextStore contextStore = new ContextStore(lissaConfiguration.contextConfigurations());
        ...
        Preprocessor sourcePreprocessor =
                Preprocessor.createPreprocessor(lissaConfiguration.sourcePreprocessor(), contextStore);
        Preprocessor targetPreprocessor =
                Preprocessor.createPreprocessor(lissaConfiguration.targetPreprocessor(), contextStore);

        EmbeddingCreator embeddingCreator =
                EmbeddingCreator.createEmbeddingCreator(lissaConfiguration.embeddingCreator(), contextStore);
        SourceElementStore sourceStore = new SourceElementStore(lissaConfiguration.sourceStore());
        TargetElementStore targetStore = new TargetElementStore(lissaConfiguration.targetStore());

        Classifier classifier = lissaConfiguration.createClassifier(contextStore);
        ResultAggregator aggregator =
                ResultAggregator.createResultAggregator(lissaConfiguration.resultAggregator(), contextStore);
        ...
        Statistics.generateStatistics(
                traceLinks, configFile, lissaConfiguration, sourceArtifacts.size(), targetArtifacts.size());
        Statistics.saveTraceLinks(traceLinks, configFile, lissaConfiguration);
